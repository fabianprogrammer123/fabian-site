(function paintJourneyLiquid(window) {
  ...
  var FIELD_FRAGMENT_SHADER = [
    ...
    '  vec3 earthShadow = pigment * vec3(0.36, 0.32, 0.38);',
    '  vec3 warmLight = mix(pigment, vec3(0.94, 0.79, 0.61), 0.30);',
    '  if (band < 0.5) return earthShadow;',
    '  if (band < 1.5) return mix(earthShadow, pigment, 0.58);',
    '  if (band < 2.5) return pigment * vec3(0.90, 0.94, 0.88);',
    '  if (band < 3.5) return mix(pigment, warmLight, 0.22);',
    '  if (band < 4.5) return pigment * vec3(0.76, 0.82, 0.88);',
    '  return warmLight;',
    '}',
    '',
    'void main() {',
